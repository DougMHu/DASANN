...
def nnToGraph(interfaces):
    ...
    for i, interface in enumerate(interfaces):
        ...
        row = interface[0]
        for inputNode in range(len(row)):
            graph.add_node((i, inputNode))
            if (i == 0):
                inputs = inputs + [(i, inputNode)]
            numNodes += 1
    ...
    for outputNode in range(len(interfaces[-1])):
        graph.add_node((i+1, outputNode))
        outputs = outputs + [(i+1, outputNode)]
        numNodes += 1
    #print graph.nodes
    #print numNodes
