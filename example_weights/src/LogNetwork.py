...
#### Define the Real Arithmetic and Log Domain Arithmetic
class RealArithmetic(object):

    @classmethod
    def add(cls, a, b):
        """Implements an Add operation on numbers a and b.
        """
        return a+b

    @classmethod
    def matrixprod(cls, x, y):
        """Matrix product of matrices x and y."""
        return np.dot(x,y)

    @classmethod
    def mac(cls, a, b):
        """Implements a Multiply ACcumulate operation (or matrix product)
        on matrix a and b.
        """
        return cls.matrixprod(a,b)

class MapArithmetic(RealArithmetic):

    @classmethod
    def mult(cls, a, b):
        """Implements a Multiply operation on numbers a and b."""
        return a*b
    ...
    def dotprod(cls, x, y):
        ...
        mult = map(cls.mult,x,y)
        return reduce(cls.add,mult)

    @classmethod
    def matrixVectorProd(cls, x, y):
        """Matrix product of Matrix x and Vector y."""
        dot = map((lambda a: cls.dotprod(a,y)),x)
        return np.array(dot)
    ...
    def matrixprod(cls, x, y):
        ...
        if (len(y.shape) == 1):
            if (len(x.shape) == 1):
                result = cls.dotprod(x,y)
            else:
                result = cls.matrixVectorProd(x,y)
        else:
            result = map((lambda a: cls.matrixVectorProd(x,a)), y.transpose())
            result = np.array(result).transpose()
        return result

class LogArithmetic(MapArithmetic):
    ...
    def mult(cls, a, b):
        ...
        B = np.log(abs(b))
        P = A + B
        if (nP):    # if the product is negative
            return -np.e**(P)
        else:
            return np.e**(P)
    ...
    def add(cls, a, b):
        ...
        if (na == nb):
            ns = na
            S = max(A,B) + np.log(1 + np.e**(-abs(A-B)))
        else:
            S = max(A,B) + np.log(1 - np.e**(-abs(A-B)))
            if (A > B):
                ns = na
                ...
                ns = nb
        if (ns):    # if the sum is negative
            return -np.e**S
        else:
            return np.e**S

class LogApproxArithmetic(LogArithmetic):
    ...
    i = np.linspace(0, N_bins, N_bins+1)

    delta_plus_table = np.log(1 + np.e**(- x_max*(i+0.5)/N_bins ))
    delta_minus_table = np.log(1 - np.e**(- x_max*(i)/N_bins ))

    @classmethod
    def delta_plus(cls, x):
        """Look up table for delta_plus offset.
        Input x MUST BE GREATER THAN 0. """
        if (x > cls.x_max):
            return cls.delta_plus_table[-1]
        else:
            index = int(cls.N_bins*x/cls.x_max)
            return cls.delta_plus_table[index]

    @classmethod
    def delta_minus(cls, x):
        """Look up table for delta_minus offset.
        Input x MUST BE GREATER THAN 0. """
        if (x > cls.x_max):
            return cls.delta_minus_table[-1]
        else:
            index = int(cls.N_bins*x/cls.x_max)
            return max(cls.delta_minus_table[index], cls.y_min);
    ...
    def add(cls, a, b):
        ...
        if (na == nb):
            ns = na
            S = max(A,B) + cls.delta_plus(abs(A-B))
        else:
            S = max(A,B) + cls.delta_minus(abs(A-B))
            if (A > B):
                ns = na
            else:
                ns = nb
...
class LogArithmetic_NoConversion(MapArithmetic):
    ...
    def add(cls, A, B):
        """Implements an Add operation on numbers a and b."""
        return np.log(np.e**A + np.e**B)

#### Main Network class
class Network(object):

    def __init__(self, sizes, cost=CrossEntropyCost, math=0, 
        sparsity=1, conn = None):
        ...
        self.cost=cost
        if (math==0):
            self.math = RealArithmetic
        elif (math==1):
            self.math = MapArithmetic
        elif (math==2):
            self.math = LogArithmetic
        elif (math==3):
            self.math = LogApproxArithmetic
        else:
            self.math = LogArithmetic_NoConversion
    ...
    def feedforward(self, a):
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            z = map(self.math.add, self.math.mac(w, a), b)
            a = sigmoid(np.array(z))
        return a
    ...
    def backprop(self, x, y):
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            z = np.array(map(self.math.add, self.math.mac(w, activation), b))
            zs.append(z)
        ...
        nabla_w[-1] = self.math.mac(delta, activations[-2].transpose())
        #print "nabla_w[-1] = {}".format(nabla_w[-1])
        ...
        for l in xrange(2, self.num_layers):
            ...
            delta = self.math.mac(self.weights[-l+1].transpose(), delta) * sp
            nabla_b[-l] = delta
            #nabla_w[-l] = np.dot(delta, activations[-l-1].transpose())
            nabla_w[-l] = self.math.mac(delta, activations[-l-1].transpose())
